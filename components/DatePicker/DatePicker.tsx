 ...
 const DatePicker: React.FC<DatePickerProps> = ({
     ...
     const goPrevMonth = () => {
         const prev = dayjs().year(viewYear).month(viewMonth).date(1).subtract(1, 'month');
         setViewYear(prev.year());
         setViewMonth(prev.month());
     };
 
     const goNextMonth = () => {
         const next = dayjs().year(viewYear).month(viewMonth).date(1).add(1, 'month');
                 ...
                 <div className={classes.popover} role="dialog" aria-modal="true">
                     ...
                     <div className={classes.days}>
                         {Array.from({ length: firstWeekdayIndex }).map((_, i) => (
                             <div key={`empty-${i}`} style={{ width: 'calc((280px - 6 * 4px) / 7)' }} />
                         ))}
                         {Array.from({ length: daysInMonth }).map((_, i) => {
                             const dayNum = i + 1;
                             const d = startOfMonth.date(dayNum);
                             const selected =
                                 parsed?.isValid() && parsed.isSame(d, 'day');
